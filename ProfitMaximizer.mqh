//+------------------------------------------------------------------+
//|              Profit Maximizer Include File for Enhanced Smart Money Trader    |
//+------------------------------------------------------------------+
// This file contains functions to enhance profit capturing capabilities
// Include this file in your main EA to enable these features
...
// Global variables for market regime
...
{
   // Get ATR values for volatility comparison
   int atr_handle = iATR(symbol, entry_tf, atr_period);
   if(atr_handle == INVALID_HANDLE) return REGIME_UNDEFINED;
   ...
   double atr_short = atr_values[0]; // Current ATR
   double atr_long = 0;              // Average of last 20 ATR values
   
   for(int i = 1; i <= 20; i++)
      atr_long += atr_values[i];
                           ...
                           double current_drawdown, bool verbose_mode)
{
   ...
   current_tp = MathMax(0.7, MathMin(current_tp, 3.5));
...
//+------------------------------------------------------------------+
double CalculateMultiStageTrailing(ENUM_POSITION_TYPE position_type, 
...
//+------------------------------------------------------------------+
void AnalyzeTradingPatterns(TradeResult &trade_history[], int size, 
                          string &active_direction, bool &correlated_pairs_active,
                          bool verbose_mode)
{
   int losing_buys = 0;
   int losing_sells = 0;
   ...
   if(total_losing >= 2 && total_analyzed >= 5)
   {
      ...
      else
      {
         ...
         if(active_direction != "" && !correlated_pairs_active)
            active_direction = "";
      }
   }
} 
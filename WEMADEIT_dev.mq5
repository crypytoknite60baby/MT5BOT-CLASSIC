...
input double RR_Ratio = 2.0;                 // Risk:Reward ratio (gold: 2.0)
input int    Slippage = 50;                  // Max slippage (gold: 50pt)
...
input double FVG_ReactionDistance = 0.5;     // ATR multiplier for FVG proximity (gold: 0.5)
input double RelativeLevelTolerance = 0.001; // Tolerance for relative equal levels
input double MinimumBodySize = 0.6;          // ATR multiplier for strong bodies (gold: 0.6)
...
input double PumpBodyATR = 1.5;               // Body >= this * ATR qualifies (gold: 1.5)
input bool   PumpBreakPrev = true;            // Require break of previous high/low

// Anti flip/whipsaw and execution hygiene
input int    AntiFlipMinutes = 60;            // Block opposite entries for N minutes after a trade (gold: 60)
...
input int    MinHoldBars = 3;                 // Minimum bars to hold before breakeven/management (gold: 3)
input double MinRRBeforeOpposite = 0.5;       // Block opposite entries until current trade reaches this RR
input int    MinSpreadPoints = 80;            // Skip entries if spread too wide (gold: 80pt)

// Relax throttles a bit
input int    MaxTradesPerSession = 3;         // Session throttle (gold: 3)
...
input int    MaxDailyTrades = 4;             // Max daily trades (gold: 4)
...
input double HRLR_StopMultiplier = 2.5;      // Wider stops in HRLR (gold: 2.5)

//--- Gold-Specific Parameters (NEW)
input bool   EnableGoldMode = true;           // Enable gold-specific tuning
input int    GoldLondonNYOverlap_Start = 13;  // London-NY overlap start (UTC)
input int    GoldLondonNYOverlap_End = 16;    // London-NY overlap end (UTC)
input bool   SkipNFP = true;                  // Skip Non-Farm Payrolls day
input bool   SkipFOMC = true;                 // Skip FOMC days
input double MaxSwapPerLot = 15.0;            // Alert when swap exceeds $/lot
...
//+------------------------------------------------------------------+
// Telegram config — override these or leave blank to disable
...
int OnInit() {
   ...
   MonitorInit();
   if(TelegramToken != "")
      MonitorInitTelegram(TelegramToken, TelegramChatID);
   MonitorLog(MON_SEV_INFO, "INIT", "WEMADEIT_dev v1.01 — Gold Mode: " +
      (EnableGoldMode ? "ON" : "OFF") + " | Symbol: " + _Symbol +
      " | Magic: " + IntegerToString(MagicNumber) +
      " | Balance: $" + DoubleToString(initial_balance, 2) +
      " | Telegram: " + (TelegramToken != "" ? "ON" : "OFF"));

   if(EnableGoldMode && StringFind(_Symbol, "XAU") < 0) {
...
bool IsInKillZone() {
   ...
   // NY Open (9:30-10:00 ET)
   if(hour==NYOpen_Start && minute>=30) return true;
   if(hour==NYOpen_End && minute>=0 && minute<=30) return true; // FIXED edge case
   ...
   // Gold-specific: London-NY overlap (best liquidity for XAUUSD)
   if(IsGoldSymbol() && hour >= GoldLondonNYOverlap_Start && hour < GoldLondonNYOverlap_End) return true;
...
// Helpers
bool IsGoldSymbol() { return EnableGoldMode && StringFind(_Symbol, "XAU") >= 0; }
...
bool SpreadOK() {
   ...
   return spread_pts <= MinSpreadPoints;
}
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| New bar handler — runs once per new candle                       |
//+------------------------------------------------------------------+
void OnNewBar() {
   // Reset daily counters
   MqlDateTime dtc, dtt; TimeToStruct(TimeCurrent(), dtc); TimeToStruct(last_trade_date, dtt);
   if(dtc.day != dtt.day || dtc.mon != dtt.mon || dtc.year != dtt.year) {
      daily_trades = 0; daily_profit = 0.0; session_trade_count = 0;
      MonitorResetDaily();
   }

   if(daily_trades >= MaxDailyTrades) { MonitorLog(MON_SEV_INFO, "LIMIT", "Daily trade limit reached"); return; }
   double daily_loss_limit = AccountInfoDouble(ACCOUNT_BALANCE) * MaxDailyLoss / 100.0;
   if(daily_profit < -daily_loss_limit) { MonitorLog(MON_SEV_WARN, "LIMIT", "Daily loss limit reached: " +
      DoubleToString(daily_profit, 2)); return; }
   if(consecutive_losses >= 2 && (TimeCurrent() - last_loss_time) < 4 * 3600) { MonitorLog(MON_SEV_INFO, "COOLDOWN",
      "Cooldown active — consecutive losses: " + IntegerToString(consecutive_losses)); return; }
   ...
   if(session_trade_count >= MaxTradesPerSession) { MonitorLog(MON_SEV_INFO, "LIMIT", "Session trade cap reached"); return; }
   if(last_trade_time != 0 && (TimeCurrent() - last_trade_time) < MinMinutesBetweenTrades * 60) { return; }
   ...
   if(bullish_signal) {
      ...
      if(UseFibEntry && PlacePendingOrders) {
         ...
         if(fibEntry < ask && !PendingExists(ORDER_TYPE_BUY_LIMIT, fibEntry, 5)) {
            ...
            MqlTradeRequest rq; MqlTradeResult rs; ZeroMemory(rq); ZeroMemory(rs);
            rq.action = TRADE_ACTION_PENDING; rq.symbol = _Symbol; rq.volume = lotSize; rq.type = ORDER_TYPE_BUY_LIMIT;
            rq.price = fibEntry; rq.sl = sl; rq.tp = tp; rq.deviation = Slippage; rq.magic = MagicNumber;
            rq.comment = "ICT_FIB_BUY";
            if(!OrderSend(rq, rs)) { MonitorLog(MON_SEV_ERROR, "ORDER", "Failed BUY_LIMIT ret=" +
               IntegerToString(rs.retcode)); }
            MonitorTrackTradeResult(rq, rs);
         }
         return;
      }
      ...
      double takeProfit = (UseDynamicTP ? (double)NearestLiquidityTP(true) : 0.0);
      if(takeProfit <= 0.0) takeProfit = entryPx + RR_Ratio * atr;
      double stopLossPips = (entryPx - stopLoss) / _Point; double lotSize = CalculateLotSize(stopLossPips);
      MqlTradeRequest req; MqlTradeResult res; ZeroMemory(req); ZeroMemory(res);
      req.action = TRADE_ACTION_DEAL; req.symbol = _Symbol; req.volume = lotSize; req.type = ORDER_TYPE_BUY;
      req.price = entryPx; req.sl = stopLoss; req.tp = takeProfit; req.deviation = Slippage; req.magic = MagicNumber;
      req.comment = "ICT_ADV_BULL";
      if(OrderSend(req, res)) {
         daily_trades++; session_trade_count++; last_trade_date = TimeCurrent();
         ...
         MonitorLog(MON_SEV_ERROR, "ORDER", "Failed BUY market ret=" +
            IntegerToString(res.retcode) + " seq=" + IntegerToString(g_mon_trade_sequence));
      }
      MonitorTrackTradeResult(req, res);
   }
   ...
   if(bearish_signal) {
      ...
      if(UseFibEntry && PlacePendingOrders) {
         ...
         if(fibEntry > bid && !PendingExists(ORDER_TYPE_SELL_LIMIT, fibEntry, 5)) {
            ...
            MqlTradeRequest rq; MqlTradeResult rs; ZeroMemory(rq); ZeroMemory(rs);
            rq.action = TRADE_ACTION_PENDING; rq.symbol = _Symbol; rq.volume = lotSize; rq.type = ORDER_TYPE_SELL_LIMIT;
            rq.price = fibEntry; rq.sl = sl; rq.tp = tp; rq.deviation = Slippage; rq.magic = MagicNumber;
            rq.comment = "ICT_FIB_SELL";
            if(!OrderSend(rq, rs)) { MonitorLog(MON_SEV_ERROR, "ORDER", "Failed SELL_LIMIT ret=" +
               IntegerToString(rs.retcode)); }
            MonitorTrackTradeResult(rq, rs);
         }
         return;
      ...
      double baseSL = entryPx + 1.0 * atr; double stopLoss = isLRLR ? baseSL : entryPx + (HRLR_StopMultiplier * atr);
      double takeProfit = (UseDynamicTP ? (double)NearestLiquidityTP(false) : 0.0);
      if(takeProfit <= 0.0) takeProfit = entryPx - RR_Ratio * atr;
      double stopLossPips = (stopLoss - entryPx) / _Point; double lotSize = CalculateLotSize(stopLossPips);
      MqlTradeRequest req; MqlTradeResult res; ZeroMemory(req); ZeroMemory(res);
      req.action = TRADE_ACTION_DEAL; req.symbol = _Symbol; req.volume = lotSize; req.type = ORDER_TYPE_SELL;
      req.price = entryPx; req.sl = stopLoss; req.tp = takeProfit; req.deviation = Slippage; req.magic = MagicNumber;
      req.comment = "ICT_ADV_BEAR";
      if(OrderSend(req, res)) {
         daily_trades++; session_trade_count++; last_trade_date = TimeCurrent();
         last_trade_time = TimeCurrent(); partial_taken = false; last_trade_dir = -1;
         ...
         MonitorLog(MON_SEV_ERROR, "ORDER", "Failed SELL market ret=" +
            IntegerToString(res.retcode) + " seq=" + IntegerToString(g_mon_trade_sequence));
      }
      MonitorTrackTradeResult(req, res);
   }
...
//+------------------------------------------------------------------+
//| Track closed trades for state variables                           |
//+------------------------------------------------------------------+
void TrackClosedTrades() {
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if(PositionGetString(POSITION_SYMBOL) != _Symbol) continue;
      return; // still has an open position
   }

   static double last_known_pnl = 0;
   if(!HistorySelect(TimeCurrent() - 3600, TimeCurrent())) return;
   int total = HistoryDealsTotal();
   if(total <= 0) return;
   ulong ticket = HistoryDealGetTicket(total - 1);
   if(!HistoryDealSelect(ticket)) return;
   if((int)HistoryDealGetInteger(ticket, DEAL_MAGIC) != MagicNumber) return;
   double profit = HistoryDealGetDouble(ticket, DEAL_PROFIT);
   if(profit == 0 || profit == last_known_pnl) return;
   last_known_pnl = profit;
   daily_profit += profit;
   if(profit < 0) {
      consecutive_losses++;
      last_loss_time = TimeCurrent();
      MonitorLog(MON_SEV_INFO, "LOSS", "Trade closed LOSS: $" +
         DoubleToString(profit, 2) + " consecutive=" + IntegerToString(consecutive_losses));
   } else {
      consecutive_losses = 0;
      MonitorLog(MON_SEV_INFO, "WIN", "Trade closed WIN: $" +
         DoubleToString(profit, 2));
   }
   partial_taken = false;
   // Estimate RR from history deal
   double entry_price = HistoryDealGetDouble(ticket, DEAL_PRICE);
   double close_price = HistoryDealGetDouble(ticket, DEAL_CLOSE);
...
//+------------------------------------------------------------------+
//| Tick handler — connection check runs EVERY tick                  |
//+------------------------------------------------------------------+
void OnTick() {
   ...
   MonitorCheckBrokerStatus();
   MonitorPeriodicCheck(MagicNumber, _Symbol);
   OnNewBar();
   ManagePositions();
} 